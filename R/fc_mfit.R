...
splvm.fit <- function(Y, fam, form,
                      control = list(method = c("ML","EM","hybrid"), 
                                     ...
                                     ghQqp = 15, iter.lim = 100, full.hess = F,
...
# Testing: Y = simR$Y; fam = fam; form = form;
#          control = list(method = "EM", start.val = lc, constraint = l1,
#          ghQqp = 15, iter.lim = 150, tol = sqrt(.Machine$double.eps), silent = F)
...
ghQ <- mvgH(n = control$ghQqp, formula = form)
...
if(!is.null(loadmt)){
 ...
 message("Constraint matrices not supplied, simple structure assumed")
 loadmt <- NULL
 for(i in parY){
...
if(!is.null(icoefs)){
  if(!is.list(icoefs)) stop("Provided starting values should be a list with dim p*(q+intercept) loading matrices for mu, sigma, tau, nu")
  if(length(icoefs) != length(parY)) stop("Number of matrices in starting values should match number of parameters mu, sigma, tau, nu")
  names(icoefs) <- parY 
  for(i in parY) {
    colnames(icoefs[[i]]) <- colnames(ghQ$out[[i]]); rownames(icoefs[[i]]) <- colnames(Y)
    if(length(icoefs[[i]]) != p.*ncol(ghQ$out[[i]])) stop("Provided starting values is not lenght p*(q+intercept), revise dimension")
 ...
 message("Starting values not supplied, set to 0.5")
...
A2 <- c(exp(rowSums(A1,dim = 2))%*%ghQ$weights) # this is efy
lln <- sum(log(A2)) # log-likelihood new
dlln <- round(lln,3)
eps <- abs(lln-llo)
iter <- iter + 1
bold <- bnew
...
if(control$silent == F){
  if(exists("catmsg")) cat(paste0("\n Using trust-region algorithm to find ML estimates ", catmsg))
...
iter <- iter + 1
bold <- bnew
if(iter > 20){ method <- "ML"; catmsg <- paste0("(after ", iter," EM iterations)") }
...
error = function(e){
print(paste("\r Error in Estimation, proceeded with next simulation \r Error:",e))
...
# pD <- exp(rowSums(A1,dim = 2))/A2 # this is EC (posterior density)
# dvL <- dvY(Y,ghQ,bold,fam) # List with all the necessary derivatives
# bnew <- upB(bold,ghQ,fam,dvL,pD,full.hess = F) #updated betas
# 
# A1 <- dY(Y,ghQ,bnew,fam)
# A2 <- c(exp(rowSums(A1,dim = 2))%*%ghQ$weights) # this is efy
# lln <- sum(log(A2)) # log-likelihood new
...
# bold <- bnew
# 
# # If trust is activated
# b2r <- lb2cb(bold)
# btr <- b2r[lb2cb(bold) != 0]
# r1 <- trust::trust(objfun = loglkf, parinit = btr, rinit = 1, rmax = 10,
#                    iterlim = 100, minimize = F, Y = Y, bg = bold, ghQ = ghQ, fam = fam)
# b2r[lb2cb(bold) != 0] <- r1$argument
# bnew <- cb2lb(b2r,bold)

...
splvm.simfit <- function(l,ga){
 ...
 tmp1 <- splvm.fit(Y = AA$Y, fam = fam, form = AA$form, control = list(method = "ML", silent = T, constraint = irestr, information = "Hessian"))
 ...
 r0 <- c(lb2cb(lc.)); r1 <- c(lb2cb(tmp1$b)); r2 <- c(lb2cb(tmp2$b))
 rlist <- c(tmp2$pml.control$lambda,tmp1$iter,tmp2$iter,tmp2$iiter,length(r1)+length(r0),length(r0)+length(r1)+length(r2))
 return(c(r0,r1,r2,GBIC(tmp1),GBIC(tmp2),GIC(tmp1),GIC(tmp2),rlist))},

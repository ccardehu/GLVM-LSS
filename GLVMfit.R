...
GLVM.fit <- function(Y, fam, form, loadmt, ghp = 10, iter.lim = 500,
                     tol = 1e-3, silent = F, icoefs = icoefs, useoptim = c("BFGS","trustR"), skipEM = T){
...
if(missing(useoptim)) useoptim <- F
...
while(eps1 > tol & iter < iter.lim){
 ...
 if("mu" %in% parY){
  ...
  #bnew$mu <- t(sapply(1:p, function(i) as.matrix(bnew$mu[i,]) - ginv(tcrossprod(Sm[[i]]))%*%Sm[[i]]))
 ...
 if("sigma" %in% parY){
  ...
  #bnew$sigma <- t(sapply(1:p, function(i) as.matrix(bnew$sigma[i,]) - ginv(tcrossprod(Ss[[i]]))%*%Ss[[i]]))
...
return(list(b = bnew, loglik = lln, loadmt = loadmt, iter = iter, gr = gr, Score = Sco, Hessian = Hes))

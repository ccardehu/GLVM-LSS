...
dFun <- function(i,z,fam,Y,Z,b){ #should be evaluated at i = 1:p; z = 10; fam = fam[i]; Y = simR$Y[,i]; Z = gr$out; b = borg
  ...
  if(fam == "ZIpoisson"){
    ...
    dZIpoisson <- function(Y,mu,sigma){
      u <- as.numeric(Y == 0)
      #u <- ifelse(Y == 0, (1 + exp(-expit(sigma)-mu))^(-1),0) #
      lf <- u*c(log(sigma + (1-sigma)*exp(-mu))) + (1-u)*(c(log(1-sigma)) - c(mu) + Y*c(log(mu)) - lfactorial(Y))
      #lf <- u*log(sigma) + (1-u)*(- mu + Y*log(mu) - lfactorial(Y)) ### Complete log-likelihood
      return(lf)
    }
...
dvFun <- function(i,z,fam,Y,Z,b){ # i = 1; z = 10; fam = fam[i]; Y = simR$Y[,i]; Z = gr$out; b = borg
 ...
 if(fam == "normal"){
  ...
  d1mu = 1/(sigma^2) * (Y - mu) * 1
  d2mu = -(1/sigma^2) * 1
  d1sg = (((Y - mu)^2 - sigma^2)/(sigma^3)) * sigma
  d2sg = -(2/(sigma^2)) * sigma^2
 ...
 if(fam == "gamma"){
  ...
  d2mu = -trigamma(mu) * mu^2
  d1sg = (Y/(sigma^2) - mu/sigma) * sigma
  d2sg = (-mu/(sigma)) * sigma^2
  dcms = rep(0, length(Y)) # the expected cross derivative mu and sigma
  rL <- list("d1mu" = d1mu, "d2mu" = d2mu, "d1sg" = d1sg, "d2sg" = d2sg, "dc" = dcms)
 }
 ...
 if(fam == "ZIpoisson"){
  ...
  d1mu = (1-u)*(Y/mu-1) * mu
  d2mu = (1-u)*(-Y/mu^2) * mu^2 # expected value; original is (-Y/mu^2)
  #d1sg = ((u - sigma)/(sigma*(1-sigma))) * (sigma*(1-sigma))
  d1sg = (u/sigma) * (sigma*(1-sigma))
  #d2sg = -(1/(sigma*(1-sigma))) * (sigma*(1-sigma))^2 #expected value; original is the same
  d2sg = -u/(sigma^2) * (sigma*(1-sigma))^2
  dcms = rep(0, length(Y))
  rL <- list("d1mu" = d1mu, "d2mu" = d2mu, "d1sg" = d1sg, "d2sg" = d2sg, "dc" = dcms)
 }
...
fFun <- function(i,fam,Z,b){ #should be evaluated at i = 1:p; fam = fam[i]; Z = as.matrix(simR$Z); b = borg
  ...
  if(fam == "ZIpoisson"){
    ...
    SY = sqrt(exp(as.matrix(Z$mu)%*%matrix(b$mu[i,]))*(1-probs(as.matrix(Z$sigma)%*%matrix(b$sigma[i,])))*(1+probs(as.matrix(Z$sigma)%*%matrix(b$sigma[i,]))*exp(as.matrix(Z$mu)%*%matrix(b$mu[i,]))))
  }
  # Add other distributions in SimFA::fod
